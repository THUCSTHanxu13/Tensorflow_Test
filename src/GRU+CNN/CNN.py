...
class LSTM_MODEL(object):
	...
	def __init__(self, is_training, word_embeddings, config):
		...
		second_input = tf.concat(2,  [output_forward, output_backward])
		

		lstm_cell = tf.nn.rnn_cell.BasicLSTMCell(size + size, forget_bias=0.0)
		...
		with tf.variable_scope("LSTM"):
			for time_step in range(num_steps):
				if time_step > 0: tf.get_variable_scope().reuse_variables()
				(cell_output, state) = cell(second_input[:, time_step, :], state)
				outputs.append(cell_output)		

		second_output = tf.expand_dims(tf.reshape(tf.concat(1,  outputs), [batch_size, -1, size + size]), -1)

		pooled_outputs = []
		for i, filter_size in enumerate(filter_sizes):
			with tf.name_scope("conv-maxpool-%s" % filter_size):
				filter_shape = [filter_size, size+size, 1, num_filters]
